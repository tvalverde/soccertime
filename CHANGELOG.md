...
This file is the **website and its API**.

The applications under `android/` keep their own, in `android/CHANGELOG.md`: they ship on
their own tags (`android-v*`) and a reader of one release has no use for the other's.
...
- **The agenda's date filter stopped converting every row in the table to compare a day.**
  ...
  **Measured against a production snapshot, and returning the identical rows every time,
  verified by primary key: a day went from 349 ms to 0.5 ms, the two-day range the phone asks
  for from 374 ms to 0.5 ms, and the paginator's count from 361 ms to 0.9 ms.** Six days were
  compared, including both days of the year the clocks move.
  ...
  sites were left on the old form**, and three of them are the ones the Android apps hit on
  every launch. They are `for_date`, `for_date_range`, the API's `date_from` and `date_to`,
  and the scraper's reconciliation window. The two API filters now delegate to the queryset
  instead of writing their own ORM, so the shape lives in one file.

  It also explains why the apps saw seven to eleven seconds where the server shows two to
  three when idle: four concurrent requests were measured taking **17.9–19.6 s each**, not
  four times two. One worker, one CPU, four threads each running the conversion over the whole
  table twice. Any second device in the house pushed a slow request into one slow enough to
  worry a health check.
