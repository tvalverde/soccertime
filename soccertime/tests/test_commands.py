...
class TestScrapitCommandProcessing:
    ...
    def test_race_event_no_duplication_on_date_shift(self, db, mock_race_event):
        ...
        initial_date = Race.objects.first().date
        ...
        assert Race.objects.count() == 1
        assert Race.objects.first().date != initial_date

    def test_multiple_sessions_same_race_are_saved_separately(self, db):
        """Each session of the same Grand Prix (Libres, Sprint, Clasificación, Carrera)
        must be stored as an independent Race record, not collapsed into one."""
        base_dt = datetime.datetime.now() + datetime.timedelta(days=1)
        sessions = ["Libres 1", "Clasificación al sprint", "Sprint", "Clasificación", "Carrera"]
        events = [
            ScrapingEvent(
                datetime=base_dt + datetime.timedelta(hours=i * 2),
                sport="Automovilismo",
                competition="Fórmula 1",
                competition_crest=None,
                channels=["DAZN"],
                details=RaceDetails(name="G.P. Canadá (Gilles Villeneuve)", details=session),
            )
        ...
        assert Race.objects.count() == len(sessions)
        saved_details = set(Race.objects.values_list("details", flat=True))
        assert saved_details == set(sessions)
    ...
    def test_simple_event_no_duplication_on_date_shift(self, db, mock_simple_event):
        ...
        initial_date = SimpleEvent.objects.first().date
